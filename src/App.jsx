...
const App = () => {
  ...
  const { isScanning, flashlight, camera, playSound, currentCode, error } = useSelector(state => state.scanner);
  const { showHistory, showSettings } = useSelector(state => state.history);

  // Mock current code for demo
  ...
  return (
    <div className="relative w-full h-screen bg-gray-900 overflow-hidden">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-30">
        <div className="flex space-x-2">
          <button
            ...
            className={`p-3 rounded-full ${flashlight ? 'bg-yellow-500' : 'bg-black/50'} text-white`}
          >
            {flashlight ? <Zap className="w-6 h-6" /> : <ZapOff className="w-6 h-6" />}
          </button>
          <button
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-30">
        ...
        >
          {isScanning ? <Pause className="w-8 h-8" /> : <Play className="w-8 h-8 ml-1" />}
        </button>
      </div>
